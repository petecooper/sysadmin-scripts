...
; then \
    echo -e '\n== nginx ==' \
    && echo -e '=> nginx found at `/usr/sbin/nginx/nginx`.' \
...
; then \
    echo -e '\n== nginx ==' \
    && echo -e '=> nginx found at `/usr/sbin/nginx`.' \
    && echo -e '=> Checking nginx version...' \
    && /usr/sbin/nginx -v \
; fi \
&& if \
...
; then \
    echo -e '\n== Caddy ==' \
    && echo -e '=> Caddy found at `/opt/caddy/caddy/caddy`.' \
...
; then \
    echo -e '\n== FrankenPHP ==' \
    && echo -e '=> FrankenPHP found at `/opt/frankenphp/frankenphp/frankenphp`.' \
