...
if [ -S "/var/run/php/php-fpm74.sock" ]; then
  ...
  if [ -d "/etc/nginx/servers-available/" ]; then
    egrep -lir "php-fpm74.sock" /etc/nginx/servers-available/ | sed -i 's/\/etc\/nginx\/servers-available\//->/g'
  fi
  if [ -d "/etc/nginx/sites-available/" ]; then
    egrep -lir "php-fpm74.sock" /etc/nginx/sites-available/ | sed -i 's/\/etc\/nginx\/sites-available\//->/g'
  fi
fi
