#!/bin/bash
if [ -S "/var/run/php/php-fpm85.sock" ]; then
  echo -e "\nFound PHP 8.5 socket at \`/var/run/php/php-fpm85.sock\`."
  if [ -d "/opt/nginx/servers-available/" ]; then
  egrep -lir "php-fpm85.sock" /opt/nginx/servers-available/ | sed 's/^/-> /g' | sort
  fi
  if [ -d "/opt/nginx/sites-available/" ]; then
  egrep -lir "php-fpm85.sock" /opt/nginx/sites-available/ | sed 's/^/-> /g' | sort
  fi
fi
